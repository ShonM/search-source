...
SearchSource.prototype._loadData = function(query, options) {
  ...
  if(this._canUseHistory(historyKey, options)) {
  ...
  function handleData(err, payload) {
    if(err) {
      ...
      if(self.options.keepHistory) {
        ...
        self._mergeInHistory(historyKey, {
          data: data,
          range: range,
          loaded: new Date(),
          metadata: metadata
      ...
      if(version > self._loadedVersion) {
        if(self.options.keepHistory){
          ...
          self._updateStore(self.history[historyKey].data);
        } else {
...
SearchSource.prototype._canUseHistory = function(historyKey, options) {
  if(this.options.keepHistory) {
